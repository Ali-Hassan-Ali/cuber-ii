        ...
        <div class="card card-primary">
          ...
          <form id="create-form">
                    ...
                    <select class="form-control" id="role" style="width: 100%;">
                        <option selected disabled>Select The Role</option>
                         ...
                         <option value="{{ $role->id }}"
                             @if($adminRole->id == $role->id ) selected @endif>{{ $role->name }}</option>
                         @endforeach
                     </select>
                </div>
              <div class="form-group">
            ...
            <div class="card-footer">
                  {{--  <a href="#" onclick="update('{{ $admin->id }}')"  class="btn btn-info">submit</i></a>  --}}
                  <button type="button"  onclick="adminUpdate()"  class="btn btn-info">update</button>
...
<script>
      ...
      function adminUpdate() {
        ...
        update('/admin/admins/{{ $admin->id }}', data, '/admin/admins/');
    }
</script>
@endsection
